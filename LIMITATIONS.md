...
## Objective
To ensure a rigorous and transparent security evaluation, this document explicitly separates production Microsoft and Aegis capabilities from their simulated representations within this local Proof of Concept (PoC) environment.
...
| **Downstream Execution** | PostgreSQL + Audit Ledger | The audit log is a standard relational table. While effective for PoC observability `(DB:0 / DB:1)`, it does not constitute a mathematically immutable ledger (e.g., blockchain or WORM storage). |

## Specific Test Artifacts & Boundary Definitions

### 1. Network Topology & Isolation (T12)
To prove network-level enforcement, the PoC splits the gateway into two explicit routes:
...
The 14-point test harness enforces a strict scoring mechanism. A scenario is only marked as `[PASS]` if **both** the Microsoft Baseline and the Aegis Proxy return their exact expected HTTP statuses and database execution counts. 
